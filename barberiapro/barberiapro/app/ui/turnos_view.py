...
def view(page: ft.Page, app_state, embed=True):
    ...
    def open_form(row=None):
        ...
        if row:
            ...
            select_barbero.value = str(row.get("barbero_id") or "")
            select_servicio.value = str(row.get("servicio_id") or "")
            fecha.value = row.get("fecha") or ""
            hora.value = row.get("hora") or ""
            title = "Editar Turno"
        else:
            select_cliente.value = select_barbero.value = select_servicio.value = None
            fecha.value = hora.value = ""
            title = "Nuevo Turno"

        def save(e):
            try:
                if not select_cliente.value or not select_barbero.value or not select_servicio.value or not fecha.value.strip():
                    page.snack_bar = ft.SnackBar(ft.Text("Completa cliente, barbero, servicio y fecha"))
                    page.snack_bar.open = True
                    page.update()
                    return
                if current:
                    models.update_turno(current["id"],
                                        int(select_cliente.value),
                                        int(select_barbero.value),
                                        int(select_servicio.value),
                                        fecha.value.strip(),
                                        hora.value.strip())
                else:
                    models.create_turno(int(select_cliente.value),
                                        int(select_barbero.value),
                                        int(select_servicio.value),
                                        fecha.value.strip(),
                                        hora.value.strip())
        ...
        content = ft.Column([
            ...
            ft.Divider(),
            select_cliente, select_barbero, select_servicio, fecha, hora,
    ...
    def delete(row):
        models.delete_turno(row["id"])
        load()

    def load():
        table.rows.clear()
        for r in models.get_turnos():
            table.rows.append(ft.DataRow(cells=[
                ft.DataCell(ft.Text(r.get("cliente") or "")),
                ft.DataCell(ft.Text(r.get("barbero") or "")),
                ft.DataCell(ft.Text(r.get("servicio") or "")),
                ft.DataCell(ft.Text(r.get("fecha") or "")),
                ft.DataCell(ft.Text(r.get("hora") or "")),
    ...
    load()
    return ft.Stack([ft.Column([ft.Row([ft.ElevatedButton("Nuevo Turno", on_click=lambda e: open_form())]), table]), sidebar])
